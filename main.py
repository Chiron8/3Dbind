...
import digitalio # Allows the MC to detect external buttons
...
# Sum 3 readings for each axis then divide by 3 to get mean average
for i in range(3):
...
while True:
    ...
    # if button is pressed simulate pressing S key, if it isn't release the S key
    if not button.value:
        kbd.press(Keycode.S)
    else:
